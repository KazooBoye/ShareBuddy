...
const purchaseCredits = async (req, res, next) => {
  try {
    ...
    const creditPackages = {
      10: 10000,  // 10 credits = 10,000 VND
      25: 20000,  // 25 credits = 20,000 VND (20% discount)
      50: 35000,  // 50 credits = 35,000 VND (30% discount)
      100: 60000  // 100 credits = 60,000 VND (40% discount)
    };

    if (!creditPackages[amount]) {
      return res.status(400).json({
        success: false,
        error: 'Gói credit không hợp lệ',
        data: {
          availablePackages: Object.keys(creditPackages).map(credits => ({
            credits: parseInt(credits),
            price: creditPackages[credits]
          }))
        }
      });
    }

    const price = creditPackages[amount];

    // TODO: Integrate with payment gateway (Momo, VNPay, etc.)
      ...
      await withTransaction(async (client) => {
        ...
        await client.query(
          `INSERT INTO credit_transactions (user_id, amount, transaction_type, description)
           VALUES ($1, $2, $3, $4)`,
          [userId, amount, 'purchase', `Mua ${amount} credits - ${paymentMethod} - ${transactionId}`]
        );
      });

      res.json({
        success: true,
        message: `Mua ${amount} credits thành công`,
        data: {
...
// Get credit packages and pricing
const getCreditPackages = async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      data: {
        packages,
        paymentMethods: ['momo', 'vnpay', 'banking']
      }
    });
  } catch (error) {
    next(error);
