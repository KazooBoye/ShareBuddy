...
const CreditSummary: React.FC<{ creditPackage: CreditPackage; currency: 'usd' | 'vnd' }> = ({ creditPackage, currency }) => {
  ...
  const formattedPrice = currency === 'usd' ? `$${price.toFixed(2)}` : `${price.toLocaleString('vi-VN')} VND`;
  return (
    <div className="mb-4 p-3 rounded border">
      <div className="d-flex justify-content-between align-items-center">
        ...
        <div className="text-end">
          <h5 className="mb-0">{formattedPrice}</h5>
        </div>
      </div>
    </div>
  );
...
const CheckoutForm: React.FC<{ selectedPackage: CreditPackage | null; onSuccess: () => void; currency: 'usd' | 'vnd' }> = ({ 
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        {processing ? <Spinner animation="border" size="sm" className="me-2" /> : `Pay ${currency === 'usd' ? '$' + selectedPackage.price_usd.toFixed(2) : selectedPackage.price_vnd.toLocaleString('vi-VN') + ' VND'}`}
      </Button>
